...
function SettingCumbi() {
  ...
  function getSettingCumbi() {
    ApiService.getSetting('').then(
      (response) => {
        if (response.status === 'success') setSetting(response.setting);
      },
      (err) => {
        console.log('err', err);
  ...
  const getTRM = (e) => {
    ...
    ApiService.getSetting('/trm').then(
      (response) => {
        if (response.status === 'success') setSetting({ ...setting, trm: response.trm, date_trm: response.date });
      },
      (err) => {
        console.log('err', err);
        console.log('err.stack', err.stack);
          ...
          <div className="row">
            ...
            <div className="col-md-3 mt-3">
              <label className="form-label">
                Last update:
                {timestamp2date(setting.date_trm)}
              </label>
